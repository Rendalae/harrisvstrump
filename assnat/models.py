...
def simple_logistic_regression(legislation_number, merge_data_sets=True, drop_col = DROP_COLS, simplify_fam = SIMPLIFY_FAM, drop_fam = DROP_FAM, na_col = NA_COLS , drop_names = DROP_NAMES , min_words= MIN_WORDS, punct_opt=PUNCT_OPT):
    if merge_data_sets:
    ...
    df = complete_preproc(df, drop_col= drop_col, simplify_fam= simplify_fam, drop_fam= drop_fam, na_col= na_col, drop_names= drop_names, min_words = min_words, punct_opt= punct_opt)
